  ...
  function onChange(e) {
    ...
    async function onSubmit(e){
      ...
      try {
        ...
        updateProfile(auth.currentUser, {  // User name
          displayName: name
        })
        const user = userCredential.user
        const formDataCopy = {...formData} // Copy of form data
        delete formDataCopy.password // Delete password from the form data 
        formDataCopy.timestamp = serverTimestamp();

        await setDoc(doc(db, "users",user.uid ), formDataCopy)  // save in a collection (firebase) called User with the same auth user UID 
      ...
      <div className="flex  justify-center flex-wrap items-center px-6 py-12 max-w-6xl mx-auto">
        ...
        <div className="w-full md:w-[67%] lg:w-[40%] lg:ml-20 ">
          <form onSubmit={onSubmit} >
            ...
            <button
              className="w-full bg-blue-600 px-7 py-3 text-white text-sm font-medium uppercase rounded shadow-md  hover:bg-blue-700 transition duration-150 ease-in-out hover:shadow-lg active:bg-blue-800"
              type="submit"
            >
              Sign Up
            </button>
